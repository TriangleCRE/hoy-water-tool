...
//
// In production the schema is created once and then exists forever, so
// paying the advisory-lock round trip on *every* cold start is pure waste —
// and worse than waste: on 2026-08-06 every cold start piled onto the lock
// at once and each one timed out waiting for it, turning a healthy database
// into a wall of /api/tenants and /api/readings 500s that looked exactly
// like the tool's data had vanished (it hadn't — the rows were untouched).
// Check for the tables first with a cheap query that needs no lock at all;
// only fall through to the create+seed path (and its lock) the one time the
// schema genuinely doesn't exist yet.
